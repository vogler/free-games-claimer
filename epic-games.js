...
const { Signale } = pkg;
const signale = new Signale({ stream: [process.stdout] }); // TODO also append to data/epic-games.log
...
signale.debug('userAgent:', await page.evaluate(() => navigator.userAgent));
signale.success('ok');
signale.info('ok');
signale.debug('ok');
signale.warn('ok');
...
while (await page.locator('a[role="button"]:has-text("Sign In")').count() > 0) { // TODO also check alternative for signed-in state
  signale.error("Not signed in anymore. Please login and then navigate to the 'Free Games' page. If using docker, open http://localhost:6080");
...
for (let i = 1; i <= n; i++) {
  ...
  if (btnText.toLowerCase() == 'in library') {
    ...
    if (await page.locator('button:has-text("Continue")').count() > 0) {
      // signale.info('Device not supported. This product is not compatible with your current device.');
      await page.click('button:has-text("Continue")');
    }
    ...
    try {
      ...
      //   signale.error('Encountered hcaptcha. Giving up :(');
      //   await page.pause();
      //   process.exit(1);
      // }
      // await page.waitForTimeout(3000);
      await page.waitForSelector('text=Thank you for buying'); // EU: wait, non-EU: wait again
      signale.success('Claimed successfully!');
    } catch (e) {
      signale.error(e);
      const p = path.resolve(dirs.screenshots, `${new Date().toISOString()}.png`);
      await page.screenshot({ path: p, fullPage: true });
      signale.info('Saved a screenshot of hcaptcha challenge to', p);
      signale.error('Got hcaptcha challenge. To avoid it, get a link from https://www.hcaptcha.com/accessibility'); // TODO save this link in config and visit it daily to set accessibility cookie to avoid captcha challenge?
    }
    // await page.pause();
  }
