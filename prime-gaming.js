...
const context = await firefox.launchPersistentContext(cfg.dir.browser, {
  ...
  recordHar: cfg.record
    ? { path: `data/record/pg-${datetime()}.har` }
...
try {
  ...
  const internal = await games
    .locator('.item-card__action:has([data-a-target="FGWPOffer"])')
    .elementHandles();
  const external = await games
    .locator('.item-card__action:has([data-a-target="ExternalOfferClaim"])')
    .all();
  console.log(
  ...
  for (const { title, url } of external_info) {
    ...
    await Promise.any([
      ...
      page.click('button:has-text("Claim now")'),
      page.click('button:has-text("Complete Claim")'),
        ...
        if (0) {
          ...
          if (store == "gog.com") {
            ...
            if (reason && reason.includes("captcha")) {
              ...
              console.log(
                "  Redeemed successfully? Please report your Responses (if new) in https://github.com/vogler/free-games-claimer/issues/5"
              );
            ...
            if (page2.url().startsWith("https://login.")) {
              console.error(
                "  Not logged in! Use the browser to login manually."
              );
              redeem_action = "redeem (login)";
            } else {
              const r = page2.waitForResponse((r) =>
                r.url().startsWith("https://purchase.mp.microsoft.com/")
              );
            ...
            try {
              await page2.waitForResponse((r) =>
                r
                  .url()
                  .startsWith(
                    "https://promo.legacygames.com/promotion-processing/order-management.php"
                  )
              );
  ...
  if (cfg.pg_claimdlc) {
    ...
    const dlcs = await Promise.all(
      cards.map(async (card) => ({
        ...
        url:
          ...
          (await card
            .locator('[data-a-target="ExternalOfferClaim"]')
            .getAttribute("href")),
      }))
    );
    // console.log(dlcs);
    ...
    for (const dlc of dlcs) {
      ...
      try {
        ...
        await Promise.any([
          page.click('button:has-text("Get in-game content")').then((r) => {}),
          page.click('button:has-text("Claim your gift")'),
          page
            .click('button:has-text("Claim now")')
            .then(() => page.click('button:has-text("Continue")')),
        ]);
        page.click('button:has-text("Continue")').catch((_) => {});
        const linkAccountButton = page
          .locator('[data-a-target="LinkAccountButton"]')
          .first();
        let unlinked_store;
